...
def plan_market(w, farm):
    ...
    feed_want = (feed_herd * 2 + 8) if feed_herd else (4 if w.day <= 2 else 0)
    if wheat_have < feed_want and cash >= w.price("WHEAT"):
        buy_n = min(max(6, feed_want - wheat_have), 24)
        cost = w.price("WHEAT") * buy_n
        if afford(cost):
            orders.append(["BUY_PRODUCT", "WHEAT", buy_n])

    # 4. Two sheep for early wool, then cows for milk (s1 never reached cows).
    ...
    # 5. First land earlier; second land once dairy cash is flowing.
    ...
    elif not endgame and extras < 2 and w.day >= 12 and w.money >= 8000:

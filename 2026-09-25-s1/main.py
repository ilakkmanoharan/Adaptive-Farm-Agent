...
def _agent_impl(obs, config=None):
    ...
    if total_land < 1 and money >= 120:
        _last_action = "TILL"
        _skip_count = 0
        return {"action": "TILL"}

    if money >= 30 and wheat < (empty_tilled + 1) and empty_tilled >= 1 and day < 0.8 * episode_length:
        _last_action = "BUY"
        _skip_count = 0
        return {"action": "BUY", "item": "wheat_seed", "n": 1}

    if money >= 200 and animal_count < 2 and _animal_buys < 2 and feed >= 3:
        _last_action = "BUY"
        _skip_count = 0
        _animal_buys += 1
...
if __name__ == "__main__":
    print("Kaggriculture 2026-09-25-s1 ready")

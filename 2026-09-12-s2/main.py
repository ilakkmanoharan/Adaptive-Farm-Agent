"""Kaggriculture submission 2 — 2026-09-12-s2.

Scale-up of the v1 planner after ladder losses (us ~20k vs opponents 116k–164k).
Same crash-safe protocol; larger herd, strawberries, land, and market feed.
"""
...
TARGET_SHEEP = 3
TARGET_MELONS = 10
TARGET_STRAWBERRY = 16
TARGET_WHEAT_TILES = 18
MAX_HANDS = 10
SHED_CAP = 100
BOARD = 10
TURNS_PER_DAY = 24
...
def max_plants(w, farm):
    ...
    return min(70, max(14, workers * 5))
...
def plan_market(w, farm):
    ...
    feed_want = max(unfed + 2, herd + 2 if herd else 0)
    if wheat_have < feed_want and cash >= w.price("WHEAT") and (herd or unfed):
        buy_n = min(feed_want - wheat_have, 8)
        cost = w.price("WHEAT") * buy_n
        if afford(cost):
            orders.append(["BUY_PRODUCT", "WHEAT", buy_n])

    # 4. Animals from day 0. Allow one unplaced purchase in the pipeline.
    ...
    if (not endgame) and w.day <= 20 and pending <= 1 and space_ok:
        cash_floor = 120 if (farm["sheep"] + farm["cows"] < 2 and w.day <= 2) else 400
        if farm["sheep"] + pending_sheep < min(2, TARGET_SHEEP) and w.day <= 6 and cash >= ANIMAL_COST["SHEEP"] + cash_floor:
            if afford(ANIMAL_COST["SHEEP"]):
                orders.append(["BUY_ANIMAL", "SHEEP", 1])
        elif farm["cows"] + pending_cows < TARGET_COWS and cash >= ANIMAL_COST["COW"] + cash_floor:
            if afford(ANIMAL_COST["COW"]):
                orders.append(["BUY_ANIMAL", "COW", 1])

    # 5. Land: NE then SW once the current footprint is tight.
    extras = max(0, len(w.quads) - 1)
    land_need = len(plantable_empty(w, farm)) <= 3
    if extras < 2 and not endgame and land_need:
        cost = LAND_PRICES[extras]
        min_cash = 1400 if extras == 0 else 2800
        if w.day >= 3 and cash >= cost + 200 and w.money >= min_cash:
            if afford(cost):
    ...
    seed_orders = []
    melon_cap = 6 if w.day <= 4 else TARGET_MELONS
    ...
    if last_plant_ok(w, "WHEAT") and ph != "LIQUIDATE":
        want = 6 if farm["wheat_tiles"] < TARGET_WHEAT_TILES else 2
        have_s = w.seed("WHEAT")
        if have_s < want:
...
def plant_choice(w, farm):
    ...
    if len(farm["plants"]) >= max_plants(w, farm):
        return None
    if len(plantable_empty(w, farm)) <= 2 and (pending_animals(w) or len(farm["animals"]) < 4):
        return None
    herd = farm["cows"] + farm["sheep"] + farm["geese"]
    if last_plant_ok(w, "MELON") and farm["melon_tiles"] < TARGET_MELONS and w.seed("MELON") > 0:
        return "MELON"
    wheat_need = max(TARGET_WHEAT_TILES, herd + 8)
    ...
    if last_plant_ok(w, "STRAWBERRY") and farm["strawberry_tiles"] < TARGET_STRAWBERRY and w.seed("STRAWBERRY") > 0:
        return "STRAWBERRY"
    if last_plant_ok(w, "WHEAT") and w.seed("WHEAT") > 0:
        return "WHEAT"
    return None

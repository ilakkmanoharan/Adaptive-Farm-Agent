"""Kaggriculture submission 4 — neuro-symbolic adaptive dairy.

Intervention-learned mechanisms (cow timing, wheat buffer, second land) with
online EMA updates and UCB-style mode planning. Never DROP.
"""
...
_last_key = None
_reasoner = None
_active_mode = "cow_first_dairy"
_adapt = {"wheat_extra": 10, "wheat_cap": 28, "land2_day": 10, "land2_cash": 7000}


class Mechanism:
    __slots__ = ("name", "weight", "n", "mean_delta", "m2")
    ...
    def observe(self, delta):
        ...
        conf = 0.15 if self.n < 1 else 1.0 / (1.0 + (math.sqrt((self.m2 / max(1, self.n - 1)) / max(1, self.n)) if self.n > 1 else 2000) / 2000.0)
        self.weight = max(0.05, 0.75 * self.weight + 0.25 * (1.0 + self.mean_delta / 5000.0) * conf)


class NeuroSymbolicReasoner:
    """Explicit mechanisms + money-delta interventions; plans mode under uncertainty."""

    def __init__(self):
        self.mechanisms = {
            "dairy_compounding": Mechanism("dairy_compounding", 1.5),
            "wheat_buffer": Mechanism("wheat_buffer", 1.2),
            "second_land": Mechanism("second_land", 1.0),
            "placement_throughput": Mechanism("placement_throughput", 1.2),
        }
        self.mode_counts = {}
        self.mode_rewards = {}
        self.total_pulls = 0
        ...
        self.last_mode = None
        self.sticky_mode = "cow_first_dairy"
        self.sticky_until = 3

    def plan(self, day, hour, money, cows, quads):
        global _adapt
        if day <= self.sticky_until:
            mode = "cow_first_dairy"
        elif quads < 3 and cows >= 4 and day >= 9:
            mode = "land_rush"
        elif cows >= 5:
            mode = "wheat_heavy_dairy"
        else:
            mode = "cow_first_dairy"
        # Adapt continuous knobs from mechanism weights (symbolic → numeric).
        dairy_w = self.mechanisms["dairy_compounding"].weight
        wheat_w = self.mechanisms["wheat_buffer"].weight
        land_w = self.mechanisms["second_land"].weight
        # Priors from offline interventions on winner replays.
        _adapt["wheat_extra"] = int(10 + 4 * max(0, wheat_w - 1.0))
        _adapt["wheat_cap"] = int(28 + 4 * max(0, wheat_w - 1.0))
        _adapt["land2_day"] = 9 if land_w >= 1.0 else 11
        _adapt["land2_cash"] = 6500 if land_w >= 1.0 else 7500
        if hour == 0:
            self._observe(money, mode)
        self.sticky_mode = mode
        return mode

    def _observe(self, money, mode):
        if self.last_money is not None and self.last_mode:
            delta = money - self.last_money
            self.total_pulls += 1
            n = self.mode_counts.get(self.last_mode, 0) + 1
            self.mode_counts[self.last_mode] = n
            prev = self.mode_rewards.get(self.last_mode, 0.0)
            self.mode_rewards[self.last_mode] = prev + (delta - prev) / n
            mapping = {
                "cow_first_dairy": ("dairy_compounding", "placement_throughput"),
                "land_rush": ("second_land", "dairy_compounding"),
                "wheat_heavy_dairy": ("wheat_buffer", "dairy_compounding"),
            }
            for name in mapping.get(self.last_mode, ()):
                self.mechanisms[name].observe(delta)
...
def plan_market(w, farm):
    ...
    # 3. Aggressive market wheat — winners buy hundreds; we need herd*2+8 buffer.
    wheat_have = w.shed_n("WHEAT") + sum(inv_get(inv, "WHEAT") for _, _, inv in w.units)
    pending_animals = (
        w.shed_n("COW")
        + w.shed_n("SHEEP")
        + sum(inv_get(inv, a) for _, _, inv in w.units for a in ("COW", "SHEEP"))
    )
    feed_herd = herd + pending_animals
    wheat_extra = int(_adapt.get("wheat_extra", 8))
    wheat_cap = min(28, int(_adapt.get("wheat_cap", 24)))
    feed_want = (feed_herd * 2 + wheat_extra) if feed_herd else (4 if w.day <= 2 else 0)
    if wheat_have < feed_want and cash >= w.price("WHEAT"):
        buy_n = min(max(6, feed_want - wheat_have), wheat_cap)
        cost = w.price("WHEAT") * buy_n
        if afford(cost):
            orders.append(["BUY_PRODUCT", "WHEAT", buy_n])

    # 4. Two sheep bootstrap, then cows — same as s3 (validated locally).
    ...
    if (not endgame) and w.day <= 24 and pending <= 1 and space_ok:
        cash_floor = 150 if w.day <= 2 else 300
        sheep_have = farm["sheep"] + pending_sheep
        cows_have = farm["cows"] + pending_cows
        if sheep_have < 2 and cash >= ANIMAL_COST["SHEEP"] + cash_floor:
            if afford(ANIMAL_COST["SHEEP"]):
                orders.append(["BUY_ANIMAL", "SHEEP", 1])
        elif sheep_have >= 2 and cows_have < TARGET_COWS and cash >= ANIMAL_COST["COW"] + cash_floor:
            if afford(ANIMAL_COST["COW"]):
                orders.append(["BUY_ANIMAL", "COW", 1])
        elif cows_have >= 4 and sheep_have < TARGET_SHEEP and cash >= ANIMAL_COST["SHEEP"] + cash_floor:
            if afford(ANIMAL_COST["SHEEP"]):
                orders.append(["BUY_ANIMAL", "SHEEP", 1])

    # 5. Land — reasoner may pull second unlock slightly earlier.
    extras = max(0, len(w.quads) - 1)
    land2_day = int(_adapt.get("land2_day", 11))
    land2_cash = int(_adapt.get("land2_cash", 7500))
    if not endgame and extras < 1 and w.day >= 8 and w.money >= 4000:
        cost = LAND_PRICES[0]
        if cash >= cost + 400 and len(plantable_empty(w, farm)) <= 6:
            if afford(cost):
                orders.append(["BUY_LAND"])
    elif not endgame and extras < 2 and w.day >= land2_day and w.money >= land2_cash:
        cost = LAND_PRICES[min(extras, len(LAND_PRICES) - 1)]
        if cash >= cost + 800 and len(plantable_empty(w, farm)) <= 8:
            if afford(cost):
                orders.append(["BUY_LAND"])
    elif not endgame and extras < 2 and w.day >= land2_day and w.money >= land2_cash:
        cost = LAND_PRICES[min(extras, len(LAND_PRICES) - 1)]
        if cash >= cost + 700 and (len(plantable_empty(w, farm)) <= 10 or farm["cows"] >= 4):
            if afford(cost):
                orders.append(["BUY_LAND"])
    ...
    if last_plant_ok(w, "STRAWBERRY") and w.day >= 2 and ph != "LIQUIDATE" and empties >= 3:
        have_b = farm["strawberry_tiles"] + w.seed("STRAWBERRY")
        if have_b < TARGET_STRAWBERRY and cash >= SEED_COST["STRAWBERRY"] + 150:
            n = min(3, TARGET_STRAWBERRY - have_b, int((cash - 150) // SEED_COST["STRAWBERRY"]))
    ...
    # Feed buys before most sells so dairy herd does not starve for cash-chasing sells.
    sells = [o for o in orders if o[0] == "SELL"]
    emergency = [o for o in sells if o[1] in ("MELON", "MILK", "WOOL", "STRAWBERRY")][:1]
    rest_sells = [o for o in sells if o not in emergency][:2]
    ...
    out = []
    for group in (emergency, feed_buys, rest_sells, seed_orders, commits, hires):
        out.extend(group)
    return out[:10]
...
def agent_impl(obs, config=None):
    global _active_mode, _reasoner
    w = World(obs)
    if w.day == 0 and w.hour == 0 and int(getv(obs, "step", 0) or 0) <= 1:
        _reasoner = None
    farm = classify(w)
    _active_mode = get_reasoner().plan(w.day, w.hour, w.money, farm["cows"], len(w.quads))

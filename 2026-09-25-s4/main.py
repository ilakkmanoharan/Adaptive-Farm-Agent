...
def _agent_impl(obs, config=None):
    ...
    if wheat > 0 and animal_count > 0:
        return {"action": "FEED", "item": "wheat", "n": 1}
    ...
    if day < 2 and money >= 120 and _animal_buys < 1:
        _animal_buys += 1
        return {"action": "BUY", "item": "cow", "n": 1}

    if money >= 30 and wheat < 3 and len(empty_tiles) > 0 and day < 0.8 * episode_length:
        return {"action": "BUY", "item": "wheat_seed", "n": 3}

    if money >= 80 and animal_count < 3 and _animal_buys < 4:
        _animal_buys += 1
        return {"action": "BUY", "item": "cow", "n": 1}

    if money >= 200 and animal_count >= 3:
...
if __name__ == "__main__":
    print("Kaggriculture 2026-09-25-s4 ready")

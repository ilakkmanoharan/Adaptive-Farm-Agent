"""Kaggriculture first submission — 2026-09-12.

Deterministic, stdlib-only, crash-proof economic planner.

Merges Grok's phased build order with ChatGPT's safety corrections:
- Observation is the source of truth (player actions resolve before market).
- Survival watering / feeding outrank yield work.
- Conservative herd (2 cows, 2 sheep) after a wheat pipeline exists.
- Price-aware market orders, not a hard-coded premium dump.
- Distance-aware terminal recall so banked coins, not inventory, decide the game.
"""
...
def desired_hands(w, farm):
    load = len(farm["plants"]) + 2 * len(farm["animals"]) + len(farm["weeds"])
    if w.day <= 1:
        target = 3 if load >= 2 else 2
    elif w.day <= 6:
        target = 5
    else:
        target = 6
    target = min(MAX_HANDS, max(target, (load + 3) // 4))
    if w.money < 250:
        target = min(target, 3)
    return target
...
def max_plants(w, farm):
    ...
    return min(16, max(8, workers * 2 + 2))
...
def plan_market(w, farm):
    ...
    # 3. Emergency feed purchase (lands in shed after this turn's actions).
    ...
    if unfed > wheat_have and cash >= w.price("WHEAT"):
        need = unfed - wheat_have
        buy_n = min(need, 4)
        cost = w.price("WHEAT") * buy_n
        if afford(cost):
            orders.append(["BUY_PRODUCT", "WHEAT", buy_n])

    # 4. Animals — appear in shed after this turn; never buy while a prior
    # purchase is still sitting unplaced (animals cannot be sold).
    ...
    wheat_ok = wheat_have + farm["wheat_tiles"] >= herd + pending_cows + pending_sheep + 3
    space_ok = len(plantable_empty(w, farm)) >= 2
    if (
        ph in ("EXPAND", "PRODUCE")
        and 7 <= w.day <= 18
        and cash >= 900
        and wheat_ok
        and space_ok
        and pending_cows + pending_sheep == 0
    ):
        if farm["cows"] < TARGET_COWS and cash >= ANIMAL_COST["COW"] + 300 and afford(ANIMAL_COST["COW"]):
            orders.append(["BUY_ANIMAL", "COW", 1])
        elif farm["sheep"] < TARGET_SHEEP and farm["cows"] >= 1 and cash >= ANIMAL_COST["SHEEP"] + 300 and afford(ANIMAL_COST["SHEEP"]):
            orders.append(["BUY_ANIMAL", "SHEEP", 1])

    # 5. Land only after the farm is actually productive and NW is tight.
    ...
    if (
        ...
        and not endgame
        and w.money >= 4000
        and len(plantable_empty(w, farm)) <= 2
        and cash >= LAND_PRICES[extras] + 800
    ):
        if afford(LAND_PRICES[extras]):
            orders.append(["BUY_LAND"])

    # 7. Hire at the start of the day (hands act next turn).
    target = desired_hands(w, farm)
    already = w.hires
    have = len(w.hands)
    while have + (len([o for o in orders if o[0] == "HIRE"])) < target and already < 10:
    ...
    seed_orders = []
    if last_plant_ok(w, "MELON") and ph != "LIQUIDATE" and farm["melon_tiles"] + w.seed("MELON") < TARGET_MELONS:
        if cash >= SEED_COST["MELON"] + 120 and afford(SEED_COST["MELON"]):
            seed_orders.append(["BUY_SEED", "MELON", 1])
    if last_plant_ok(w, "WHEAT") and ph != "LIQUIDATE":
        want = 3 if farm["wheat_tiles"] < TARGET_WHEAT_TILES + max(herd, 2) else 1
        have_s = w.seed("WHEAT")
        if have_s < want:
            n = min(want - have_s, 4, int(cash // SEED_COST["WHEAT"]) if cash >= SEED_COST["WHEAT"] else 0)
...
def plant_choice(w, farm):
    ...
    if last_plant_ok(w, "MELON") and farm["melon_tiles"] < TARGET_MELONS and w.seed("MELON") > 0:
        return "MELON"
    wheat_cap = TARGET_WHEAT_TILES + farm["cows"] + farm["sheep"] + farm["geese"]
    if last_plant_ok(w, "WHEAT") and w.seed("WHEAT") > 0 and farm["wheat_tiles"] < wheat_cap:
        return "WHEAT"
    return None
...
def local_action(w, farm, pos, inv, reserved_seeds):
    ...
    if kind == "PLANT":
        ...
        if (
            ...
            and crop in ("MELON", "WHEAT")
            and not ONGOING.get(crop)
        ):
            age = w.day - int(getv(tile, "planted_day", 0) or 0)
            window = (MAX_YIELD_DAY.get(crop, 4) + 1) // 2
            if window <= age <= MAX_YIELD_DAY.get(crop, 4):
...
def need_drop(w, inv):
    ...
    if goods <= 0:
        return False
    # Keep wheat if we still need to feed.
    only_wheat = goods == inv_get(inv, "WHEAT") and inv_get(inv, "WHEAT") <= 2
    if only_wheat and phase(w) not in ("LIQUIDATE",):
        return False
    ...
    if goods >= 4:
        return True
    if w.hour >= 20:
        return False  # EOD auto-drop
    if w.shed_used() >= 90:
        return False
    return goods >= 2 and w.hour <= 18
...
def build_targets(w, farm):
    ...
    # Shed logistics.
    for sx, sy in shed_tiles(w.board):
        targets.append((2000, sx, sy, "shed"))
...
def assign(w, farm, targets):
    ...
    for uid, pos, inv in w.units:
        ...
        if not claim:
            continue
        cx, cy, tag = claim
        if (cx, cy) not in valid_targets and tag not in ("shed", "drop", "pickup"):
...
def unit_action(w, farm, uid, pos, inv, assignment, reserved_seeds, seed_budget):
    if is_shed_adj(pos, w.board):
        ...
        if unfed > 0 and w.shed_n("WHEAT") > 0 and inv_get(inv, "WHEAT") == 0:
            return ["PICKUP", "WHEAT", min(3, unfed, w.shed_n("WHEAT"))]
        if unfed == 0:
    ...
    # Endgame recall: if remaining turns are tight, go drop.
    if phase(w) == "LIQUIDATE" and inv_count(inv) > 0:
        if is_shed_adj(pos, w.board):
            return ["DROP"]
        return step_toward(pos, nearest_shed(pos, w.board))
    if w.remain <= manhattan(pos, nearest_shed(pos, w.board)) + 3 and inv_count(inv) > 0:
        if is_shed_adj(pos, w.board):
            return ["DROP"]
        return step_toward(pos, nearest_shed(pos, w.board))
    ...
    if tag == "drop" or (need_drop(w, inv) and is_shed_adj(pos, w.board)):
        if is_shed_adj(pos, w.board) and inv_count(inv) > 0:
            return ["DROP"]
    if tag in ("pickup", "shed") or tag == "feed":
        if is_shed_adj(pos, w.board):
            if w.shed_n("COW") > 0 and inv_get(inv, "COW") == 0:
                return ["PICKUP", "COW", 1]
            if w.shed_n("SHEEP") > 0 and inv_get(inv, "SHEEP") == 0:

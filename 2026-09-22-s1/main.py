...
def _agent_impl(obs, config=None):
    ...
    if _animals < 2 and _money >= 50 and _wheat_seeds >= 3:
        _last_action = "BUY"
        return {"action": "BUY", "item": "animal", "n": 1}

    if _money >= 120 and _owned_land >= (_animals + 2) and _wheat_seeds >= 4:
        _last_action = "BUY"
        return {"action": "BUY", "item": "land", "n": 1}

    if _money >= 80 and _wheat_seeds < 6 and _owned_land > 0 and not weeds:
